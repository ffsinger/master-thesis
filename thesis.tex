...
We also consider a hybrid version, as described in \autoref{sec:hybrid_cnn}, where only the last five layers after flattening are in the encrypted domain. In this case, the two first $\tanh$ layers do not need to be approximated and the learning rate is a bit different.
...
However, if a latency of $176$ minutes (in the case of SEAL) can be tolerated, the time per inference is only of about $1.3$ seconds with a batch of $8192$ pages.
