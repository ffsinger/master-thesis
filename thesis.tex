...
First, we formalize the threats related to machine learning and then attempt to solve the problems of secure model training and private inference using homomomorphic encryption.
In particular, we use logistic regression models for the training problem, and neural networks for inference.
...
In \autoref{chap:background}, we will introduce two different types of ML models, namely logistic regression and neural networks, and the basics of homomorphic encryption. 
...
This is a problem, because a linear layer connects every input neuron with every output neuron, which results in a quadratic number of operations. 
...
The basic idea of homomorphic encryption is to allow computations on encrypted data, which is not possible with most modern and commonly used schemes.
This concept has been introduced in 1978 by the authors of the RSA cryptosystem \cite{rivest_data_1978}.
The first schemes supported only one type of operation, such as homomorphic additions with the Paillier cryptosystem (1999) \cite{paillier_public-key_1999}.
...
\begin{itemize}
    \item \texttt{KeyGen(params)}: Generate a public key and a private key.
    \item \texttt{Encode(x)}: Encode a vector of complex numbers into a plaintext, which is a polynomial in the ring $R = \mathbb{Z}[X]/(X^N + 1)$.
    \item \texttt{Decode(x)}: Decode a plaintext back into a vector.
    \item \texttt{Encrypt(x, pub\_key)}: Using the public key, encrypt a plaintext into an element of $R_{q_L}^2$ where $R_{q_L} = R/{q_L}R$ is the set of polynomials from $R$ with coefficients modulo ${q_L}$.
    \item \texttt{Decrypt(x, sec\_key)}: Decrypt a ciphertext using the private key.
    \item \texttt{Add(x, y)}: Pairwise addition of two ciphertexts, or one ciphertext and one plaintext.
    \item \texttt{Multiply(x, y)}: Pairwise multiplication of two ciphertexts/plaintext.
    \item \texttt{Rotate(x, k, rot\_key)}: Permute the elements in a ciphertext by shifting them by $k$ positions to the left.
    \item \texttt{Relinearize(x, rel\_key)}: After a multiplication, the resulting ciphertext will be in $R_{q_l}^3$ and will continue to grow after each multiplication. 
    Relinearization is used to bring it back to $R_{q_l}^2$ and facilitate decryption.
    It requires a relinearization key, sometimes also called an evaluation key.
    \item \texttt{Rescale(x)}: Rounding operation performed to maintain the size of a ciphertext after a multiplication. It computes $\lfloor \frac{q_{l'}}{q_l} \cdot x \rceil \in R_{q_{l'}}^2$ for a ciphertext $x \in R_{q_l}^2$.
    \item \texttt{Bootstrap(x)}: Refresh a ciphertext once it reached its maximum number of operations, by putting it back in $R_{q_L}^2$. Details are available in \cite{cheon_bootstrapping_2018} and \cite{bossuat_efficient_2021}.
\end{itemize}
During the initialization phase, some parameters need to be set:
\begin{itemize}
  \item The \emph{ring degree} $N$ determines the size of the vectors that we encode.
  It should be a power of two.
  A higher degree also provides more security.
  \item The \emph{moduli chain} is a sequence of integers $\{ q_0, ..., q_L \}$. 
  The number $L$ should be at least the multiplicative depth of the function to be computed, because one element is usually "consumed" for each rescaling operation, and one more is needed during decryption.
  \item The \emph{scaling factor} $\Delta$ of the plaintext, influencing the precision of the result.
  It is used to discretize real numbers during encoding, and should not be confused with the rescaling operation.
\end{itemize}
...
\emph{Multiparty Homomorphic Encryption} (MHE) is the second approach suggested by Mouchet et al. \cite{mouchet_multiparty_2021}.
...
We also describe our solutions to some of these issues from a high-level perspective, and explain the important design decisions made for these solutions.
...
\subsection{Threats on ML systems}\label{sec:threats}

From surveys of the ML security literature \cite{liu_survey_2018, rigaki_survey_2021}, we can identify many common types of threats.

\subsubsection{Poisoning attacks}

In many cases, an attacker wants to influence the training process by injecting or modifying the input data.
...
\subsubsection{Evasion/impersonation attacks}

For this type, the attack vector is the same but it involves carefully crafted inputs in order to produce a specific result. 
These attacks are often carried out to evade spam filters or to impersonate someone in a biometric recognition system.

\subsubsection{Data ordering attacks}
...
If the attacker has unlimited access to the prediction results, it is possible to reconstruct and steal an approximation of the model. 
...
This family contains multiple subcategories, but the general goal is to extract some knowledge from the training dataset. 
It can either be general or target a single individual.

A common type is \textbf{membership inference}, where the adversary tries to determine if a known sample was part of the training data. 
In some cases, being part of a dataset is a sensitive information in itself, such as for criminal or medical records.
The intuition behind this attack is the following.
If a model returns a high confidence value for a sample, this sample is likely to have been encountered during training.
This is due to the degree of overfitting that usually appears in practice when training ML models.

Sometimes, only part of a sample is known and the goal is to find a sensitive attribute. 
This is called an \textbf{attribute inference} or \textbf{model inversion attack}.
For instance, an attacker may want to infer the medical conditions of a person, using some known attributes (age, gender, weight) and the result of a classifier trained on this data.

To be successful, the attacker sometimes requires data poisoning \cite{hidano_model_2017}, or needs access to additional information, such as confidence levels or auxiliary datasets \cite{fredrikson_model_2015, wang_variational_2022}.

Finally, \textbf{property inference attacks} aim to leak general features of a dataset, even if these are not explicitly encoded and are not part of the learning task.
This is problematic because the data subject is usually not aware of the type of information that can be derived.


\subsubsection{Other threats}

Referring back to the dataflow diagram in \autoref{fig:dataflow}, we see that most of these attacks occur on the trust boundary around the database, and the one around the client.
...
\begin{table}[h!]
  \begin{center}
    ...
    \hline
    1. Generate keys &  \\
    2. Send relinearization and rotation keys &  \\
    3. Encrypt training data &  \\
...
For maximal efficiency, we need to be careful to minimize the total number of operations, as well as the number of chained multiplications (multiplicative depth).
...
However, the weights of the model can be encrypted and sent to the client. 
...
These first layers are often generic and sometimes rely on publicly available pre-trained weights.
...
Thus, a hybrid solution alternating between cleartext and encrypted parts could provide an important performance improvement at the expense of a reasonably small security sacrifice (the increased chance of a model extraction attack).
Although more computing power is required from the client, they keep the same guarantees in terms of data privacy.
It should however be assessed whether the divulgation of these weights represents a privacy risk from the perspective of the training data owner.
Indeed, it could facilitate some of the attacks mentioned in \autoref{sec:threats}, such as membership inference or model inversion attacks.

The architecture of a hybrid model is illustrated in \autoref{fig:hybrid_cnn}.
...
This means that no approximations are necessary for non-polynomial functions, and that popular layer types like ReLU or max-pooling are available.
...
Thus, we still need to implement our polynomial functions manually. Finally, multi-party HE is supported, but there is no bootstrapping procedure.
...
However, it does not offer bootstrapping, nor a multi-party version.
...
Similarly to logistic regression, a class wraps the encryption, prediction and decryption functions. 
...
\begin{itemize}
  \item Fully connected layer of any size
  \item 2D convolution with zero-padding
  \item 2D average pooling with stride
...
Solutions to these problems include having domain knowledge or using auxiliary datasets to find good parameter values.
...
The following evaluation has first been carried out on a small dataset of $80$ training samples and $20$ test samples. 
...
We first observe that the encrypted pipelines are 3 to 4 orders of magnitude slower than the baselines.
...
The model performance metrics (\emph{accuracy} and \emph{area under the curve}) are not different from the baseline (equal to 1 for the small dataset and close to 1 for the big one).

The approximate memory usage is almost identical for the two datasets.
...
\autoref{table:lr_steps} breaks down the runtime at every step (initialization, encryption, training, prediction, and decryption) of the different backends for the small dataset.
...
\begin{table}[h!]
  \begin{center}
    ...
    \vspace*{2mm}
    \pgfkeys{/pgf/number format/.cd, fixed, fixed zerofill, precision=4, 1000 sep={ }}
...
For this setting, we omit the PALISADE pipeline and show only the SEAL and plaintext pipelines with both packing methods, along with a baseline using the \texttt{pytorch} library.
...
We observe that the serialization step takes a considerable amount of time because the data and the keys take a lot of memory.
...
This is although not the case for the plaintext pipeline, which indicates that the convolutional layer is still the slowest by far, at least in the plaintext domain.
...
The packing method used is what we call \emph{pixelwise packing} in this work.
They also replace max-pooling layers with scaled mean-pooling layers, and ReLU with a square activation function.
They choose YASHE as the encryption scheme, which is not commonly used today.
It inspired many other works that attempted to include optimizations to alleviate the performance and memory limitations.

For instance, Badawi et al. \cite{badawi_towards_2020} implement an optimization using GPUs and evaluate their results on both the MNIST and CIFAR-10 datasets.
In \cite{badawi_privft_2020}, they design an algorithm for a specific network architecture of 2 fully connected layers (\emph{fasttext} for text classification). 
...
It also chooses an optimal matrix-vector multiplication algorithm based on the packing.
In our framework, the packing type does not change across the layers, because it would be difficult to select automatically.

Lee et al. \cite{lee_privacy-preserving_2021} focus on a maximal accuracy by using a deep model (ResNet-20) on CIFAR-10.
It is one of the few works that uses bootstrapping.
...
Compared to all these solutions, our design is meant to be flexible to allow finding the desired trade-off between performance and utility.
...
It is always good to have a variety of models at hand, because each has its own advantages and disadvantages.
...
Even by protecting only the inference phase, it is still prohibitively expensive for many use-cases involving deep networks and big images.
