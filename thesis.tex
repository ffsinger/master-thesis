...
This time, if we consider the client to be the data owner and the server to be the model owner, the steps are very similar to section \ref{sec:secure_training}:
\begin{enumerate}
...
Both methods have their own trade-offs that highly depend on the architecture of the network. They are evaluated side by side in chapter \ref{chap:evaluation}.
...
Bindings \cite{hugang_seal-python_2022}
