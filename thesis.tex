...
\begin{itemize}
    ...
    \item \texttt{Encrypt}: Using the public key, encrypt a plaintext into an element of $R_{q_L}^2$ where $R_{q_L} = R/{q_L}R$ is the set of polynomials from $R$ with coefficients modulo ${q_L}$.
    \item \texttt{Decrypt}: Decrypt a ciphertext using the private key.
