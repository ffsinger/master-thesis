...
More specifically, we will see how HE can protect data during both the training step and the inference step, and what it entails to transform an ML algorithm to a homomorphically-friendly version.
...
Now that the main mathematical and cryptographic concepts have been introduced, we focus on the problems that need to be solved.
...
One of the most influential is CryptoNets \cite{dowlin_cryptonets_2016}, which is a 5-layers CNN evaluated on the MNIST dataset. It inspired many other works that attempted to include optimizations to alleviate the strong performance limitations.
...
For more readability, this version does not support padding. The output channels are thus smaller than the input.
      ...
      \For{$l=1,...,W_{in}$}{
        ...
        \For{$i=1,...,R_{out}$}{
          \For{$j=1,...,C_{out}$}{
            ...
            $d_{bl} \leftarrow \mathbf{Add}(d_{bl}, z)$\;
          }
        }
      }
      Let $y$ be a vector filled with the bias value for channel $m$\;
      $d_{bl} \leftarrow \mathbf{Add}(d_{bl}, y)$\;
    }
  }
